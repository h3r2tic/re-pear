...
uniform float NoiseFrameOffset;
...
float PCF_Filter(float2 uv, float depth, float scale, float filterRadiusUV, float2 receiverPlaneDepthBias, float penumbra, float2 rotationTrig, float2 random2)
{
	float sum = 0.0f;
#if defined(UNITY_REVERSED_Z)
	...
	{
		float2 offset = (PoissonOffsets[i] + random2) * filterRadiusUV * scale;
...
float PCSS_Main(float4 coords, float2 receiverPlaneDepthBias, float random, float2 random2, float scale)
{
	float2 uv = coords.xy;
	...
	// STEP 3: filtering
	float shadow = PCF_Filter(uv, depth, scale, filterRadiusUV, receiverPlaneDepthBias, penumbra, rotationTrig, random2);
...
//#if defined(USE_NOISE_TEX)
	float random = tex2D(_NoiseTexture, i.uv.xy * NoiseCoords.xy * _ScreenParams.xy + frac(NoiseFrameOffset * 7.731)).a;
	random = mad(random, 2.0, -1.0);
    float2 random2 = float2(frac(random * 13.13435), frac(random * 131.72451)) * 0.25;
//#else
...
//#endif
	...
	float shadow = PCSS_Main(coord, receiverPlaneDepthBias, random, random2, scale);
...
//#if !defined(SHADOWS_SINGLE_CASCADE)
	...
	if (alpha > 1.0 - CascadeBlendDistance)
	{
		...
		float shadowNextCascade = PCSS_Main(coord, receiverPlaneDepthBias, random, random2, scale);
